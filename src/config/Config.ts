...
export const ConfigLive = Layer.effect(
  ...
  Effect.gen(function* () {
    const discord = yield* Effect.all({
      ...
      Effect.mapError(
        (error) =>
          new ConfigError({ message: `Missing Discord config: ${error}` }),
      ),
    );

    const sandbox = yield* Effect.all({
      timeout: EffectConfig.number("SANDBOX_TIMEOUT").pipe(
        Effect.orElseSucceed(() => 300000),
      ), // 5 minutes default
      maxMemory: EffectConfig.number("SANDBOX_MAX_MEMORY").pipe(
        Effect.orElseSucceed(() => 1024),
      ), // 1GB default
      maxCpus: EffectConfig.number("SANDBOX_MAX_CPUS").pipe(
        Effect.orElseSucceed(() => 2),
      ), // 2 CPUs default
    }).pipe(
      Effect.mapError(
        (error) =>
          new ConfigError({ message: `Missing Sandbox config: ${error}` }),
      ),
    );
    ...
    yield* Schema.decode(AppConfigSchema)(config).pipe(
      Effect.mapError(
        (error) =>
          new ConfigError({ message: `Config validation failed: ${error}` }),
      ),
    );
