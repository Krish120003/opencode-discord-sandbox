...
export declare namespace Command {
     ...
     * @category models
     */
    type ParseConfig<A extends Config> = Types.Simplify<{
        readonly [Key in keyof A]: ParseConfigValue<A[Key]>;
    }>;
    type ParseConfigValue<A> = A extends ReadonlyArray<infer _> ? {
        readonly [Key in keyof A]: ParseConfigValue<A[Key]>;
    } : A extends Args<infer Value> ? Value : A extends Options<infer Value> ? Value : A extends Config ? ParseConfig<A> : never;
...
export declare const fromDescriptor: {
    ...
    <A extends {
        readonly name: string;
    }, R, E>(handler: (_: A) => Effect<void, E, R>): (command: Descriptor.Command<A>) => Command<A["name"], R, E, A>;
    <A extends {
        readonly name: string;
    }>(descriptor: Descriptor.Command<A>): Command<A["name"], never, never, A>;
    <A extends {
        readonly name: string;
    }, R, E>(descriptor: Descriptor.Command<A>, handler: (_: A) => Effect<void, E, R>): Command<A["name"], R, E, A>;
};
/**
 * @since 1.0.0
 * @category accessors
...
export declare const make: {
    ...
    <Name extends string, const Config extends Command.Config>(name: Name, config: Config): Command<Name, never, never, Types.Simplify<Command.ParseConfig<Config>>>;
    <Name extends string, const Config extends Command.Config, R, E>(name: Name, config: Config, handler: (_: Types.Simplify<Command.ParseConfig<Config>>) => Effect<void, E, R>): Command<Name, R, E, Types.Simplify<Command.ParseConfig<Config>>>;
};
/**
 * @since 1.0.0
 * @category constructors
 */
export declare const prompt: <Name extends string, A, R, E>(name: Name, prompt: Prompt<A>, handler: (_: A) => Effect<void, E, R>) => Command<string, R, E, A>;
/**
 * @since 1.0.0
...
export declare const transformHandler: {
    ...
    <Name extends string, R, E, A, R2, E2>(self: Command<Name, R, E, A>, f: (effect: Effect<void, E, R>, config: A) => Effect<void, E2, R2>): Command<Name, R | R2, E | E2, A>;
};
/**
 * @since 1.0.0
 * @category combinators
...
export declare const wizard: {
    ...
    <Name extends string, R, E, A>(self: Command<Name, R, E, A>, prefix: ReadonlyArray<string>, config: CliConfig): Effect<Array<string>, QuitException | ValidationError, FileSystem | Path | Terminal>;
};
/**
 * @since 1.0.0
