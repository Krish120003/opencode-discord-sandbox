...
const parseInternal = (self, args, config) => {
  switch (self._tag) {
    case "Standard":
      {
        const parseCommandLine = args => Arr.matchLeft(args, {
          onEmpty: () => {
    ...
    case "GetUserInput":
      {
        return InternalPrompt.run(self.prompt).pipe(Effect.catchTag("QuitException", e => Effect.die(e)), Effect.map(value => InternalCommandDirective.userDefined(Arr.drop(args, 1), {
          name: self.name,
          value
        })));
      }
