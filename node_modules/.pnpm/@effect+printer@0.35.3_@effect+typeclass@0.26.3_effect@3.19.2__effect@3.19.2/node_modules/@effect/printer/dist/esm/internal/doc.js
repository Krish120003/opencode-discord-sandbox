...
const flattenSafe = self => Effect.gen(function* (_) {
  switch (self._tag) {
    case "Fail":
      {
        return self;
...
const changesUponFlatteningSafe = self => Effect.gen(function* (_) {
  switch (self._tag) {
    case "Fail":
    case "Line":
      {
    ...
    case "Nest":
      {
        return yield* _(changesUponFlatteningSafe(self.doc), Effect.map(InternalFlatten.map(doc => nest(doc, self.indent))));
      }
    case "Union":
      {
    ...
    case "Annotated":
      {
        return yield* _(changesUponFlatteningSafe(self.doc), Effect.map(InternalFlatten.map(doc => annotate(doc, self.annotation))));
      }
  }
});
// -----------------------------------------------------------------------------
